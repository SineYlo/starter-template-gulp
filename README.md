...
1. `npm run lint:html` - starts checking all html files in the specified directory for errors using w3c-validator.
2. `npm run root:clean` - runs a command that clears the dist directory.
3. `npm run image:fav` - this command compresses the size of favicons.
4. `npm run image:svg` - runs a command that optimizes svg images and also creates a sprite.
5. `npm run image:jpg` - runs a command that optimizes jpg images and converts them to webp and avif formats.
6. `npm run image:png` - runs a command that optimizes png images and converts them to webp and avif formats.
7. `npm run lint:editorconfig` - runs a command that checks all files for errors by .editorconfig.
8. `npm run build:archive` - this command creates an archive with the project, which you can then safely upload or send somewhere, for example.
9. `npm run build:watch` - runs a command that optimizes script and style files, and also starts the server.
10. `npm run build:dev` - runs a command that optimizes script and style files, but does not start the server.
11. `npm run build:prod` - runs a command that optimizes script and style files, as well as prepares these files for production.
12. `npm run build:full-start` - this command allows you to run a full project build cycle together with image optimization, but without starting the server (IMPORTANT! after running this command, you need to additionally go to the folder with the project, for example `dist`, then find the `sprite.svg` file there and delete it from it at the very top of the viewBox, otherwise the site may not display correctly).
